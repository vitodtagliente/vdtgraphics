#include <vdtgraphics/texture_viewer.h>
...
namespace graphics
{
	TextureViewer::TextureViewer(Context* const context)
	{
		...
		m_bitmap_program = createProgram(ShaderLibrary::names::TextTextureShader);
	...
	void TextureViewer::draw(Texture* const texture, const Color& color)
	{
		...
		{
			currentProgram = m_bitmap_program.get();
		}
		currentProgram->bind();
	...
	void TextureViewer::draw(const unsigned int texture, const unsigned int channels, const Color& color)
	{
		m_renderable->bind();
